      ...
      def perform_create_post
        ...
        if @args[:archetype] == Archetype.private_message
           ...
           plugin_target_users_array_J.each do |plugin_target_user_str_J|
             ...
             plugin_target_secondary_group_J = JSON.parse(plugin_target_user_J.custom_fields['secondary_group'])
             plugin_source_secondary_group_J = JSON.parse(self.user.custom_fields['secondary_group'])
             if(!((plugin_source_secondary_group_J & plugin_target_secondary_group_J).empty? ))
    ...
    UserSerializer.class_eval do
      ...
      def can_send_private_message_to_user
        ...
        if(scope.can_send_private_message?(object) && scope.current_user != object )
            plugin_target_secondary_group_J = JSON.parse(scope.current_user.custom_fields['secondary_group'])
            plugin_source_secondary_group_J = JSON.parse(object.custom_fields['secondary_group'])
            if(!((plugin_source_secondary_group_J & plugin_target_secondary_group_J).empty? ))
                 plugin_can_send_PM_J = true
   ...
   Search.class_eval do
     ...
     def user_search
      ...
      plugin_secondary_group_J.each do |plugin_temp_secondary_group_J|
        plugin_where_query_array_J <<  " '\""+plugin_temp_secondary_group_J+"\"' = ANY (ARRAY(select * from json_array_elements(cast(value AS json)))::text[]) "
      end
     ...
     def posts_query(limit, opts = nil)
      ...
      posts = Post.where(post_type: Topic.visible_post_types(@guardian.user))       
        ...
        .joins(' INNER JOIN user_custom_fields ON "user_custom_fields"."user_id"="posts"."user_id"')  
      ...
      plugin_where_query_J =plugin_where_query_array_J.join("OR")
      posts =  posts.where(plugin_where_query_J)
